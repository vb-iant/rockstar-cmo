...
// raw WP categories it shares with this one (weighted higher, since a
// shared issue/series/topic category is a stronger signal than authorship)
// plus a bonus for sharing the same author. Ties broken by recency.
...
export function getRelatedPosts(post, limit = 3) {
  ...
  const scored = candidates
    .map((p) => {
      ...
      return { post: p, score: sharedCategories * 2 + sameAuthor };
    })
    .filter((s) => s.score > 0);
